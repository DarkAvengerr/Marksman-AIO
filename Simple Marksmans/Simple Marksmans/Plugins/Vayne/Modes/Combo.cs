...
namespace Simple_Marksmans.Plugins.Vayne.Modes
{
    internal class Combo : Vayne
    {
        ...
        {
            if (IsPostAttack && Q.IsReady() && Settings.Combo.UseQ && (Settings.Combo.UseQOnlyToProcW ? (Orbwalker.LastTarget is AIHeroClient && HasSilverDebuff((AIHeroClient)Orbwalker.LastTarget) && GetSilverDebuff((AIHeroClient)Orbwalker.LastTarget).Count == 1) : true))
            {
            ...
            if (R.IsReady() && Settings.Combo.UseR)
            {
                ...
                if (target != null && Orbwalker.LastTarget is AIHeroClient && enemies >= 3 &&
